...
config.debug = process.env.GEKKO_DEBUG || true; // for additional logging / debugging

config.headless = process.env.GEKKO_HEADLESS || true;

config.api = {
  host: process.env.GEKKO_API_HOST || '127.0.0.1',
  port: process.env.GEKKO_API_PORT || 3000,
  timeout: process.env.GEKKO_API_TIMEOUT || 120000 // 2 minutes
};
...
config.workspaceSync = {
  enabled: process.env.GEKKO_WORKSPACE_SYNC_ENABLED || true,
  syncInMilis: process.env.GEKKO_WORKSPACE_SYNC_IN_MILIS || 1000 * 60 * 60 * 24,
  scm: process.env.GEKKO_WORKSPACE_SCM || 'https://github.com/jspare-projects/cloud-gekko-workspace-template.git',
  scmUser: process.env.GEKKO_WORKSPACE_SCM_USER || '',
  scmPassword: process.env.GEKKO_WORKSPACE_SCM_PASSWORD || '',
};
...
config.adapter = process.env.GEKKO_ADAPTER || 'sqlite';
...
config.mongodb = {
  ...
  version: 0.1,
  connectionString: process.env.GEKKO_MONGODB_CONNECTION
    || 'mongodb://localhost/gekko', // connection to mongodb server
  dependencies: [{
    module: 'mongojs',
    version: '2.4.0'
  }]
...
if (process.env.GEKKO_REDIS_BEACON_ENABLED && "false"
  !== process.env.GEKKO_REDIS_BEACON_ENABLED) {

  config.redisBeacon = {
    enabled: process.env.GEKKO_REDIS_BEACON_ENABLED,
    port: process.env.GEKKO_REDIS_BEACON_PORT || 6379, // redis default
    host: process.env.GEKKO_REDIS_BEACON_HOST || '127.0.0.1', // localhost
    // On default Gekko broadcasts
    // events in the channel with
    // the name of the event, set
    // an optional prefix to the
    // channel name.
    channelPrefix: process.env.REDIS_BEACON_CHANNEL_PREFIX || '',
    broadcast: [
      'candle'
    ]
  };
...
config.slack = {
  enabled: process.env.GEKKO_SLACK_ENABLED || false,
  token: process.env.GEKKO_SLACK_TOKEN || '',
  sendMessageOnStart: true,
  muteSoft: true,
  channel: process.env.GEKKO_SLACK_CHANNEL || 'tradebot' // #tradebot
}
...
config.backtestAdapterExporter = {
  enabled: true
};

config.adviceWriter = {
  enabled: process.env.ADVCIE_WRITER || false,
  muteSoft: true,
}
...
config['I understand that Gekko only automates MY OWN trading strategies'] = process.env.GEKKO_AGGREMENT
  || false;
